...
if [ -n "$REMOTE_ENV" ] && [ -f "$REMOTE_ENV" ]; then
  ...
  # Auto-generated safe defaults. These satisfy the lolian/superapp
  # compose file's `${VAR:?...}` mandatory substitutions so the stack
  # comes up without an externally-rendered .env. The caller should
  # always pass a real .env (via app_env_file / app_env_artifact_name /
  # secrets.app_env) for any non-demo deployment — the secrets below are
  # ephemeral, demo-only, and printed into the VM's startup log so
  # anyone who can read the log can read the secrets.
  cat > .env <<ENV
# Auto-generated by deploy-compose-on-vm.sh when no APP_ENV_FILE was provided.
# Replace before any non-demo use.
...
JWT_EXPIRES_IN=7d
MIN_AGE=21
# Mock-LLM by default so the assistant works without an OpenRouter key.
# Pass a real OPENROUTER_API_KEY via APP_ENV_FILE to enable live LLM calls.
MOCK_LLM=true
TOP_K=5
